...
export const APP_VERSION = "5.3.21";
...
export function getSurfaceEquivalentDiameter(length, width) {
  const l = number(length, 0);
  const w = number(width, 0);
  if (l <= 0 || w <= 0) return null;
  const surfaceArea = l * w;
  return xlRound(1.1284 * Math.sqrt(surfaceArea), 1);
}
...
export function calculateKiln(rawInput = {}) {
  ...
  // Effective size:
  // Round glass uses its entered diameter.
  // Rectangular glass uses the diameter of a circle with the same surface area:
  // diameter = 1.1284 × sqrt(length × width).
  let effectiveSize;
  if (length > 0 && width > 0) {
    effectiveSize = getSurfaceEquivalentDiameter(length, width);
  } else if (length > 0) {
  ...
  return {
    ...
    glass, effectiveSize, topTemperature, effectiveThickness,
...
export function validateInput(rawInput = {}) {
  ...
  if (input.shapeMode === "round") {
    ...
    const equivalentDiameter = getSurfaceEquivalentDiameter(length, width);
    if (equivalentDiameter && equivalentDiameter > PARAMETERS.maxDiameter) {
      warnings.push(`Calculated surface-equivalent diameter (${equivalentDiameter} cm) exceeds the validation limit of ${PARAMETERS.maxDiameter} cm.`);
    }
  }
